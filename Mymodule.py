...
def zqip():
    ...
    version = 1.0
    print("\033[31m[Console]:\033[0m","Mymodule库版本为",version)
    print("\033[31m[Console]:\033[0m","正在爬取IP,请等待")
    ...
    for line in r.readlines():
                if line == '\n':
                    line = line.strip("\n")
                file2.write(line)
    print("\033[31m[Console]:\033[0m","正在将IP写入文本")
    r.close()
    file2.close()
    #创建目录 来自https://cloud.tencent.com/developer/article/1406443
    print("\033[31m[Console]:\033[0m","检测是否存在输出目录")
    def mkdir(path):
        # 去除首位空格
        path=path.strip()
        # 去除尾部 \ 符号
        ...
        if not isExists:
            # 创建目录操作函数
            print("\033[31m[Console]:\033[0m","正在创建输出目录/export/")
            os.makedirs(path)
    ...
    dk.close()
    #删除不必要文件
    print("\033[31m[Console]:\033[0m","正在删除不必要文件")
    os.remove("file2.txt")
    os.remove("IP.txt")